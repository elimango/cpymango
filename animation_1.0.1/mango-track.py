...
mangoTrack = None
squash, isSquashed, depth = set(), False, 0
sourceList = []
    ...
    for obj in target:
        ...
        for sourceTrack in prv:
            if depth == 0:
                ...
                squash = sorted(squash)
                print("Squashed Keys from all Tracks: \n", squash, "\n")
                #for _, _, nxtCurve, prvCurve in IterateCurves(nxt, sourceTrack):
                    #DiffTracks(nxtCurve, prvCurve, None, squash)          
            if depth == 1:
                ListTracks(nxt, prv, sourceTrack)
                #print("Source Track: {}".format(sourceTrack.GetName()))

def ListTracks(nxt, prv, sourceTrack):
    isMatching, isIdentical, isUnique, = False, False, False
    prvNames, nxtNames = [], []
    global squash, depth, isSquashed
    if len(squash) > 0:
        isSquashed = True
    for name in prv:
    ...
    for i, targetTrack, nxtCurve, prvCurve in IterateCurves(nxt, sourceTrack): 
        ...
        if depth == 0 and not isSquashed:
            squash |= set(StagePrevKeys(prvCurve))
...
def DiffTracks(nxtCurve, prvCurve, nxtkeys=None, prvkeys=None):
    if not prvkeys:
        prvkeys = StagePrevKeys(prvCurve)
    if not nxtkeys:
        nxtkeys = StageNextKeys(nxtCurve)
...
def StagePrevKeys(prvCurve):
    prvkeys = []
    prvkey = 0
    while prvkey < prvCurve.GetKeyCount():
...
class MangoTrackWrapper(gui.GeDialog):
    ...
    def InitValues(self):
        ...
        global selection
        if selection:   
            for d in ObjectSelection(doc) if len(ObjectSelection(doc)) > 1 else []:
                ie.InsertObject(d, 0)      
